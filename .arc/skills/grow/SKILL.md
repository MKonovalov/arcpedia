---
name: grow
description: Grow arcpedia — plan, implement, verify, commit
tools: [bash, read_file, write_file, edit_file]
---

# Growth

You are growing arcpedia — a wiki for the agent age — from its founding vision into a working product.

## Phase 1: Understand
- Read ARC.md for project context, phased roadmap, tech stack, build commands
- Read arcpedia-concept.md for the north star vision
- Read llm-wiki.md for the spiritual ancestor (Karpathy's LLM Wiki pattern)
- Read .arc/journal.md for session history
- Read .arc/learnings.md for project-specific insights
- Check open issues for feature requests

## Phase 2: Plan
- Compare what exists to the arc.md roadmap and arcpedia-concept.md vision
...
- Commit each task: `git add -A && git commit -m "arc: [summary]"`

## Phase 4: Communicate
- Write issue responses (gh issue comment)
- Append session summary to .arc/journal.md
- Reflect → append to .arc/learnings.md if lesson learned

## Safety rules

- **Never modify llm-wiki.md.** That's the founding prompt — immutable.
- **Never modify arcpedia-concept.md.** That's the north star vision — immutable.
- **Never modify arc.md.** That's the project context.
- **Never modify .github/workflows/.** That's the automation safety net.
- **Never modify .arc/scripts/.** That's the harness.
- **Never modify .arc/config.toml.** That's the build configuration.
- **Never modify .arc/.gitignore.**
